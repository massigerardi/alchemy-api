...
func TestEthClient_GetGasPrice(t *testing.T) {
  type fields struct {
    client jsonrpc.RPCClient
  }
  tests := []struct {
    ...
    {name: "Test Success", fields: fields{client: mocks.GetMockClient()}, want: big.NewInt(11831650940)},
  }
  for _, tt := range tests {
    t.Run(tt.name, func(t *testing.T) {
      c := EthClient{
      ...
      got, err := c.GetGasPrice()
      if (err != nil) != tt.wantErr {
        t.Errorf("GetGasPrice() error = %v, wantErr %v", err, tt.wantErr)
        return
      }
      if !reflect.DeepEqual(got, tt.want) {
